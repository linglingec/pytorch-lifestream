...
def get_dataloader(data_path, numeric_cols, categorical_cols, time_col, max_past_events, last_n_transactions, batch_size, num_workers, train):
    """
    ...
    dataset = NPPRDataset(
        ...
        last_n_transactions=last_n_transactions,
        train=train
    )
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=train, num_workers=num_workers, collate_fn=dataset.collate_fn)

    return dataloader

class InferenceCallback(Callback):
    def __init__(self, checkpoint_callback, train_clf_dataloader, test_clf_dataloader, train_clf_path, test_clf_path,
                 train_target_path, results_path, embedding_dims, embedding_size, hidden_size_enc, hidden_size_dec,
        ...
        Args:
            ...
            results_path: Path to txt file with inference results.
